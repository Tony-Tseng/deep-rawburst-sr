...
class MergeBlockUNetDiff(nn.Module):
    def __init__(self, input_dim, project_dim, 
        ...
        weight_predictor = []
        weight_predictor.append(blocks.conv_block(input_dim, 2 * input_dim, 3,
                                                  stride=1, padding=1, batch_norm=use_bn, activation=activation))

        for _ in range(num_weight_predictor_res):
            weight_predictor.append(blocks.ResBlock(2 * input_dim, 2 * input_dim, stride=1,
                                                    batch_norm=use_bn, activation=activation))

        weight_predictor.append(blocks.conv_block(2 * input_dim, input_dim, 3, stride=1, padding=1,
                                                  batch_norm=use_bn,
                                                  activation='none'))

        self.weight_predictor = nn.Sequential(*weight_predictor)
        self.up = nn.Conv2d(input_dim, project_dim, 3, 1, 1)
...
class ResPixShuffleConv(nn.Module):
    ...
    def __init__(self, input_dim, num_feat, num_pre_res_blocks, post_conv_dim,
                 num_post_res_blocks,
                 use_bn=False, activation='relu',
                 upsample_factor=2, icnrinit=False, gauss_blur_sd=None, gauss_ksz=3):
        super().__init__()
        self.gauss_ksz = gauss_ksz
        self.init_layer = blocks.conv_block(input_dim, input_dim, 3, stride=1, padding=1, batch_norm=use_bn,
                                            activation=activation)

        pre_res_layers = []
        for _ in range(num_pre_res_blocks):
            pre_res_layers.append(blocks.ResBlock(input_dim, input_dim, stride=1, batch_norm=use_bn, activation=activation))

        self.pre_res_layers = nn.Sequential(*pre_res_layers)

        self.upsample_layer_4 = PixShuffleUpsampler(input_dim, input_dim, upsample_factor=upsample_factor//2,
        ...
        self.skipup1 = nn.Conv2d(input_dim, num_feat * 4, 3, 1, 1, bias=True)
        self.skipup2 = nn.Conv2d(num_feat* 4, 3, 3, 1, 1, bias=True)

        self.upconv1 = nn.Conv2d(input_dim, num_feat * 4, 3, 1, 1, bias=True)
        ...
        #     post_res_layers.append(blocks.ResBlock(input_dim, post_conv_dim, stride=1, batch_norm=use_bn,
        #                                            activation=activation))
    ...
    def forward(self, x, feat):
        ...
        # print(self.upsample_layer(x[0]).shape)
        # skip1 = self.lrelu(self.upsample_layer(self.skipup1(self.upsample_layer(x))))
        # skip2 = self.upsample_layer(self.skipup2(skip1))

        # return skip2

        feat = self.upsample_layer(feat)
        feat = self.pre_res_layers(self.init_layer(feat)) + feat
        feat = self.lrelu(self.upsample_layer(self.upconv1(feat)))
        print(feat.shape)
        
        # out = self.upsample_layer_4(out) + skip1

        # pred = self.predictor(self.post_res_layers(out)) + skip2
        # return pred
...
@model_constructor
def dcnsrnet_unet_mergediff(alignment_init_dim, reduction, dec_num_feat, 
    ...
    fusion = MergeBlockUNetDiff(input_dim=64, project_dim=240)
    decoder = ResPixShuffleConv(60, dec_num_feat, dec_num_pre_res_blocks,
