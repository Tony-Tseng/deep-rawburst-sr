...
class MergeBlockUNetDiff(nn.Module):
    def __init__(self, input_dim, project_dim, 
                 ...
                 activation='relu', efficient_out = 112):
        super().__init__()
        
        weight_predictor = []
        weight_predictor.append(blocks.conv_block(efficient_out, 2 * project_dim, 3,
        ...
        self.efficientnet_extractor = torch.hub.load('NVIDIA/DeepLearningExamples:torchhub', 'nvidia_efficientnet_b0', pretrained=True).layers[:5]
        conv_weight1 = self.efficientnet_extractor[1].block0.depsep.conv.weight.data
        self.efficientnet_extractor[1].block0.depsep.conv = nn.Conv2d(96, 96, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=96, bias=False)
        self.efficientnet_extractor[1].block0.depsep.conv.weight.data = conv_weight1

        conv_weight2 = self.efficientnet_extractor[2].block0.depsep.conv.weight.data
        self.efficientnet_extractor[2].block0.depsep.conv = nn.Conv2d(144, 144, kernel_size=(5, 5), stride=(1, 1), padding=(2, 2), groups=144, bias=False)
        self.efficientnet_extractor[2].block0.depsep.conv.weight.data = conv_weight2

        conv_weight3 = self.efficientnet_extractor[3].block0.depsep.conv.weight.data
        self.efficientnet_extractor[3].block0.depsep.conv = nn.Conv2d(240, 240, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=240, bias=False)
        self.efficientnet_extractor[3].block0.depsep.conv.weight.data = conv_weight3

        self.conv2 = nn.Sequential(nn.Conv2d(64, 32, kernel_size=3, padding=1, bias=bias))
    ...
    def forward(self, feat):
        ...
        for i in range(feat.shape[0]):
            burst_feat = self.conv2(feat[i, ...])       # (14, num_features, H/2, W/2)
            burst_feat = self.efficientnet_extractor(burst_feat)

            weight = self.weight_predictor(burst_feat)
            weights_norm = F.softmax(weight, dim=0)
            feat_weighted = (feat[i, ...] * weights_norm).sum(dim=0)
            fused_feat.append(self.up(feat_weighted))
...
class BSRT_ResPixUpSample(nn.Module):
    def __init__(self, num_in_ch, num_out_ch, num_feat, embed_dim):
        ...
        self.skipup2 = nn.Conv2d(num_feat, num_out_ch * 4, 3, 1, 1, bias=True)
...
class DCNSRNet(nn.Module):
    ...
    def forward(self, im):
        ...
        out_dec = self.decoder(im, out_fus)
