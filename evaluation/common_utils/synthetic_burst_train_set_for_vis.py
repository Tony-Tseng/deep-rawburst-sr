import torch, random
import evaluation.common_utils.synthetic_burst_generation as syn_burst_utils
...
class SyntheticBurst(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, base_dataset, burst_size=8, crop_sz=384, downsample_factor=4, add_noise = True, apply_median=False, transform=tfm.ToTensor()):
        ...
        self.image_processing_params = {'random_ccm': True, 'random_gains': True, 'smoothstep': True,
                                        'gamma': True,
                                        'add_noise': add_noise, 'apply_median': apply_median}
    ...
    def __getitem__(self, index):
        ...
        frame_width, frame_height = frame.shape[1], frame.shape[2]
        frame_crop = frame[:, (frame_width-crop_sz)//2:(frame_width+crop_sz)//2, (frame_height-crop_sz)//2:(frame_height+crop_sz)//2]
