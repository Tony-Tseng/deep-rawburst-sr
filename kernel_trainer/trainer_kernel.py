...
from kernel_utils import data_utils
from kernel_trainer.trainer import Trainer
import numpy as np
...
class Trainer_Kernel(Trainer):
    ...
    def train(self):
        ...
        for batch, data in enumerate(self.loader_train, 1):
            frame_gt = torch.tensor(np.array([np.concatenate(([data['frame_gt'][0].numpy()],[data['frame_gt'][0].numpy()],[data['frame_gt'][0].numpy()],[data['frame_gt'][0].numpy()],
                                [data['frame_gt'][0].numpy()],[data['frame_gt'][0].numpy()],[data['frame_gt'][0].numpy()],[data['frame_gt'][0].numpy()])
                                ,0)]))
            for idx in range(1, data['burst'].shape[0]):
                img = np.array([np.concatenate(([data['frame_gt'][idx].numpy()],[data['frame_gt'][idx].numpy()],[data['frame_gt'][idx].numpy()],[data['frame_gt'][idx].numpy()],
                                [data['frame_gt'][idx].numpy()],[data['frame_gt'][idx].numpy()],[data['frame_gt'][idx].numpy()],[data['frame_gt'][idx].numpy()])
                                ,0)])
                frame_gt = torch.cat((frame_gt, torch.tensor(img)), 0)
            ...
            if (batch + 1) % self.args.print_every == 0:
                self.ckp.write_log('[{}/{}]\tLoss : [total: {:.4f}]{}[mid: {:.4f}]'.format(
                    ...
                    len(self.loader_train) * self.args.batch_size,
