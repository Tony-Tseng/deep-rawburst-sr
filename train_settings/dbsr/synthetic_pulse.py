...
from models.loss.SphericalOptimizer import SphericalOptimizer
...
def run(settings):
    ...
    # latent = torch.randn((settings.batch_size, 1, 512), dtype=torch.float, requires_grad=True, device='cuda')
    ...
    #     res = (settings.batch_size, 1, 2**(i//2+2), 2**(i//2+2))
    ...
    # opt = SphericalOptimizer(optimizer, var_list, lr=2e-4)
