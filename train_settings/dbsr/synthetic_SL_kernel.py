...
import actors.dbsr_actors as dbsr_actor
...
def run(settings):
    ...
    objective = {'rgb': PixelWiseError(metric='l1', boundary_ignore=40), 'psnr': PSNR(boundary_ignore=10), 
                 'ssl': PixelWiseError(metric='l1', boundary_ignore=10)}

    loss_weight = {'rgb': 1.0}

    actor = dbsr_actor.DBSRkernelSyntheticActor(net=net, degrad_net=degrad_net, kernel_net=kernel_net, objective=objective, loss_weight=loss_weight)
