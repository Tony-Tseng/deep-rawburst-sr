...
def run(settings):
    ...
    bi = 40
    objective = {
        'rgb':  PixelWiseError(metric='l1', boundary_ignore=bi), 'psnr': PSNR(boundary_ignore=bi),
    ...
    actor = ssl_actors.DBSRRealkernelSSLActor(net=net, degrad_net=degrad_net, kernel_net=kernel_net, objective=objective, loss_weight=loss_weight, alignment_net=pwcnet)
